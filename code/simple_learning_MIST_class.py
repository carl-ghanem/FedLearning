...
class Personal_Net():
    def __init__(self):
        self.clf = nNet()
        self.loss_func = nn.CrossEntropyLoss() # maybe use another loss function, understand this one

        self.optimizer = torch.optim.SGD(
            self.clf.parameters(), lr=cfg["learning_rate"], momentum=cfg["momentum"]
        )
    ...
    def train(self, epoch, nP):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
            ...
            if cfg["manual_SGD"]:
                with torch.no_grad(): # reduce computation cost as we don't execute backprop in this loop
    ...
    def test(self, epoch, append_to_list=False):
        ...
        with torch.no_grad():
            for batch_x, batch_y in test_loader:
                ...
                correct += pred.eq(batch_y.data.view_as(pred)).sum().item()
